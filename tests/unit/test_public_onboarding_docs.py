...
def test_readme_includes_public_first_run_quickstart_and_boundary() -> None:
    ...
    assert "`synapse doctor`" in readme
    assert "`synapse runs submit <spec_path> --mode sync --stop-at SPEC_VALIDATION`" in readme
    assert "`synapse runs show <run_id>`" in readme
