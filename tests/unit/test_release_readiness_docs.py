...
def test_release_readiness_docs_cover_public_surface_and_preview_boundary() -> None:
    ...
    assert "`synapse runs show <run_id> --preview report`" in readme
    assert "`synapse runs show <run_id> --preview PLAN.clean`" in readme
...
def test_changelog_and_release_notes_capture_phase_2_technical_release() -> None:
    ...
    assert "`synapse doctor`" in changelog
    assert "`synapse runs submit <spec_path>`" in changelog
    assert "`synapse runs show <run_id> --preview report`" in changelog

    assert "# Phase 2 Technical Release" in release_notes
    assert "Synapse-Flow" in release_notes
    assert "`synapse doctor`" in release_notes
    assert "`synapse runs show <run_id> --preview report`" in release_notes
