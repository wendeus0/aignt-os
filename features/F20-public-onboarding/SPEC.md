...
constraints:
  ...
  - "explicitar no onboarding que `aignt doctor` e diagnostico local e advisory, nao preflight operacional completo"
...
acceptance_criteria:
  ...
  - "O onboarding explica explicitamente o boundary entre `aignt doctor` e `repo-preflight`, incluindo quando o doctor basta e quando o operador deve escalar para preflight operacional."
...
non_goals:
  ...
  - "alterar o comportamento de `aignt doctor`, `aignt runs submit` ou `aignt runs show`"
...
Entregar um onboarding publico curto e oficial para a primeira run, capaz de orientar um operador externo pelo caminho canonico atual da CLI e de explicitar o boundary entre diagnostico local (`aignt doctor`) e preflight operacional (`repo-preflight`) sem alterar o comportamento dos comandos existentes.
...
- troubleshooting essencial para `aignt doctor`, `aignt runs submit` e `aignt runs show`
...
3. O onboarding deve usar `aignt doctor` como diagnostico local inicial e deixar explicito que ele nao substitui `repo-preflight`.
...
- operador interpreta `aignt doctor` como garantia de ambiente completo para Docker ou runtime persistente
...
- Entao ele encontra uma orientacao explicita de que `aignt doctor` nao substitui `repo-preflight`
