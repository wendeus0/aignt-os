...
- SynapseOS continua como meta-orquestrador CLI-first; o Synapse-Flow segue como a engine propria de pipeline do projeto.
...
- A F13 introduziu a primeira saida enriquecida com Rich em `src/`, mantendo o Synapse-Flow como a engine propria de pipeline do SynapseOS e limitando o recorte a `synapse runtime status`.
- A F14 adicionou observabilidade CLI-first sobre runs persistidas com `synapse runs list` e `synapse runs show <run_id>`, reaproveitando `RunRepository` e `ArtifactStore` sem abrir TUI.
...
- A `F15-public-run-submission` foi concluida e mergeada em `main`: a CLI agora expõe `synapse runs submit <spec_path>` com `--mode auto|sync|async` e `--stop-at`, reaproveitando o `RunDispatchService` interno sem alterar schema nem abrir nova service layer.
- A `F17-artifact-preview` ja foi mergeada em `main`, adicionando preview textual controlado de `RUN_REPORT.md` e `clean_output` por step em `synapse runs show <run_id> --preview <target>`.
...
- O baseline atual tambem ja incorpora `F28-adapter-circuit-breaker`, `F29-auth-rbac-foundation` e `F30-auth-registry-cli`, com `synapse auth init|issue|disable` e o alinhamento de `docs/IDEAS.md`/README ao estado pos-F30.
...
- O Codex opera em fluxo container-first via `./scripts/dev-codex.sh`, separado do servico `synapse-os`, onde o Synapse-Flow roda como engine propria de pipeline do SynapseOS.
...
- Os artefatos operacionais padrao em `.synapse-os/` devem permanecer fora do versionamento.
